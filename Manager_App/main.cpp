...
int g_listWidth = 350;
std::vector<int> g_listCols = {110, 130, 55, 60, 70, 80, 100};
...
void ExtractApkMetadataAndIcon(const std::string& apkPath, json& appNode) {
    ...
    if (!parser.Parse(absApkPath)) return;

    if (!parser.packageName.empty()) {
        appNode["package_name"] = parser.packageName;
    }
    ...
    if (!parser.label.empty()) {
        ...
        if (currentName.empty() || currentName.rfind("unknown", 0) == 0 || currentName.find(".apk") != std::string::npos) {
            appNode["name"] = parser.label;
        }
    }
    if (!parser.description.empty()) {
        appNode["description"] = parser.description;
    }

    if (!parser.versionName.empty() && appNode.contains("versions") && !appNode["versions"].empty()) {
        if (appNode["versions"][0].value("version", "") == "1.0" || appNode["versions"][0].value("version", "") == "v1.0") {
            appNode["versions"][0]["version"] = parser.versionName;
        }
    }
...
bool IsValidApk(const std::string& path) {
    mz_zip_archive zip;
    memset(&zip, 0, sizeof(zip));
    if (mz_zip_reader_init_file(&zip, path.c_str(), 0)) {
        int idx = mz_zip_reader_locate_file(&zip, "AndroidManifest.xml", NULL, 0);
...
void RefreshAppList() {
    ...
    if (fs::exists(GetApkDir())) {
        for (const auto& entry : fs::directory_iterator(GetApkDir())) {
            ...
            if (ext == ".bak") {
                if (IsValidApk(fullPath)) {
                    std::string newPath = entry.path().parent_path().string() + "\\" + entry.path().stem().string() + ".apk";
                    try {
                        fs::rename(fullPath, newPath);
                        fullPath = newPath;
                        apkName = entry.path().stem().string() + ".apk";
                        ext = ".apk";
                    } catch (...) {}
                }
            }
    ...
    for (size_t i = 0; i < dbCache["apps"].size(); i++) {
        ...
        std::string catStr = app.value("category", "");
        std::string tagStr = "";
        if (app.contains("tags") && app["tags"].is_array()) {
            for (size_t t = 0; t < app["tags"].size(); t++) {
                tagStr += app["tags"][t].get<std::string>();
                if (t < app["tags"].size() - 1) tagStr += ", ";
            }
        }
        ...
        ListView_SetItemText(hwndApps, idx, 5, (LPSTR)catStr.c_str());
        ListView_SetItemText(hwndApps, idx, 6, (LPSTR)tagStr.c_str());
    }
}
...
svrPtr->Post("/api/disconnect", [](const httplib::Request& req, httplib::Response& res) {
    ...
    svrPtr->Get("/", [](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string html = R"(
            ...
            <html>
            <head>
                <title>Elite App Marketplace.v2</title>
            ...
            <body>
                <div class="container">
                    <h1>Elite App Marketplace.v2</h1>
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_DROPFILES: {
        ...
        for (UINT i = 0; i < numFiles; ++i) {
            ...
            } else if (ext == ".apk" || ext == ".bak") {
                if (ext == ".bak") {
                    if (IsValidApk(path)) {
                        std::string newPath = p.substr(0, dot) + ".apk";
                        try {
                            fs::rename(p, newPath);
        ...
        if (addedScreenshot && !screenshots.empty()) {
    ...
    case WM_CREATE: {
        ...
        lvc.iSubItem = 5; lvc.pszText = (LPSTR)"Category"; lvc.cx = g_listCols[5]; ListView_InsertColumn(hwndApps, 5, &lvc);
        lvc.iSubItem = 6; lvc.pszText = (LPSTR)"Tags"; lvc.cx = g_listCols[6]; ListView_InsertColumn(hwndApps, 6, &lvc);
